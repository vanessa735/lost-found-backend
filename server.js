...
const io = new Server(server, {
  cors: {
    origin: (origin, cb) => {
      ...
      if (
        EXPLICIT_ORIGINS_FOR_IO.includes(origin) ||
...
// ── Online Users Store ─────────────────────────────────────────────
// Map<userId, { socketId, name, avatar, lastSeen }>
...
io.on('connection', (socket) => {
  ...
  socket.on('user:online', ({ userId, userName }) => {
    ...
    console.log(`[Socket.IO] User online: ${userName} (${userId})`);
  });
  ...
  socket.on('conversation:join', ({ conversationId }) => {
    ...
    console.log(`[Socket.IO] ${socket.data.userId} joined room: conversation:${conversationId}`);
  });
  ...
  // ── Message sent (broadcast to room) ─────────────────────────
  ...
  socket.on('message:reaction', ({ conversationId, messageId, reaction, userId }) => {
    socket.to(`conversation:${conversationId}`).emit('message:reaction:update', {
      conversationId,
      messageId,
      reaction,
      userId,
  ...
  socket.on('messages:read', ({ conversationId, userId }) => {
    socket.to(`conversation:${conversationId}`).emit('messages:read:update', {
      conversationId,
      userId,
    });
  });
...
async function boot() {
  ...
  server.listen(PORT, () => {
    ...
    const subRaw   = 'API Server  ·  Express + MySQL + Socket.IO';
    ...
    const infoRows = [
      ...
      ['⚡ Socket.IO',  clr(C.bMagenta,'enabled (ws + polling)')],
