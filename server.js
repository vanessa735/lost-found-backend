...
//  ✅ Generous timeouts to prevent rapid reconnect loops
// ═══════════════════════════════════════════════════════════════════
...
const io = new Server(server, {
  ...
  // ✅ polling first — clients MUST start here, then upgrade
  transports: ['polling', 'websocket'],

  // ✅ Timeouts tuned for Render's free tier (cold starts are slow)
  pingTimeout:    60000,   // 60s before declaring dead
  pingInterval:   25000,   // heartbeat every 25s
  upgradeTimeout: 30000,   // time allowed to complete ws upgrade
  connectTimeout: 45000,   // total handshake budget
...
io.on('connection', (socket) => {
  ...
  // Upgrade event — useful for debugging
  socket.conn.on('upgrade', (transport) => {
    console.log(`[Socket.IO] Upgraded: ${socket.id} → ${transport.name}`);
  });
  ...
  socket.on('typing:start', ({ conversationId, userId, userName }) => {
    socket
      ...
      .emit('typing:update', { conversationId, userId, userName, typing: true });
  });
  ...
  socket.on('message:reaction', ({ conversationId, messageId, reaction, userId }) => {
    socket
      .to(`conv:${conversationId}`)
      .emit('message:reaction:update', { conversationId, messageId, reaction, userId });
  });
  ...
  res.send = function (body) {
    ...
    if (status >= 400) {
      ...
      stats.errorLog.unshift({
        method: req.method, path: req.path, status, ms,
    ...
    stats.recent.unshift({
      ...
      ms: `${ms}ms`, time: new Date().toISOString(),
    });
...
function mountRoutes() {
  ...
  routes.forEach(({ prefix, file, label }) => {
    try {
      ...
      console.log(
        `     ${clr(C.gray,`Stack: ${err.stack?.split('\n')[1]?.trim() || 'N/A'}`)}`
      );
    }
  });
  ...
  if (FAILED_ROUTES.length > 0) {
    console.log(`  ${clr(C.bRed,`✘ ${FAILED_ROUTES.length} routes FAILED:`)}`);
    FAILED_ROUTES.forEach((f) =>
      console.log(
        `     ${clr(C.bRed,'•')} ${clr(C.bYellow,f.label)} (${f.file}): ${clr(C.red,f.error)}`
      )
    );
  }
  console.log('');
...
app.get('/api/health', async (_req, res) => {
  ...
  return res.status(dbOk?200:503).json({
    ...
    realtime:  { socket_io:'enabled', online_users:onlineUsers.size, transports:'polling → websocket upgrade' },
...
app.get('/api/online-users', (_req, res) =>
  res.json({ success:true, count:onlineUsers.size, data:Array.from(onlineUsers.values()) })
);
...
async function boot() {
  ...
  server.listen(PORT, () => {
    ...
    console.log(
      clr(C.bBlue, '║') +
      ' '.repeat(tp) + bold(clr(C.bCyan, title)) +
      ' '.repeat(BW - 2 - tp - title.length) +
      clr(C.bBlue, '║')
    );
    console.log(
      clr(C.bBlue, '║') +
      ' '.repeat(sp) + dim(clr(C.gray, sub)) +
      ' '.repeat(BW - 2 - sp - sub.length) +
      clr(C.bBlue, '║')
    );
    console.log(clr(C.bBlue, row()));
    console.log(clr(C.bBlue, `╠${line}╣`));
    ...
    [
      ...
      ['⚡ Socket.IO',  clr(C.cyan,   'polling → websocket (auto-upgrade)')],
