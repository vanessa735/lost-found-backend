...
const getTransporter = () => {
  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
    console.error('[PasswordReset] EMAIL_USER or EMAIL_PASS missing!');
    return null;
  }
...
exports.verifyResetCode = async (req, res) => {
  try {
    ...
    if (token) {
      ...
      if (!email) return res.status(400).json({ success: false, message: 'Email required with code' });
      const cleanEmail = String(email).trim().toLowerCase();
      const [rows] = await db.query(
        `SELECT pr.*, u.email FROM password_resets pr
         JOIN users u ON pr.user_id = u.id
      ...
      if (!rows.length) return res.status(400).json({ success: false, message: 'Invalid code' });
      record = rows[0];
    }
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    if (!rows.length) return res.status(400).json({ success: false, message: 'Invalid or used reset link' });
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background: #f9fafb; padding: 30px; border-radius: 20px; margin-top: 20px;">
              ...
              <p style="color: #6b7280;">Your FindIt password was changed on <strong>${new Date().toLocaleString()}</strong>.</p>
